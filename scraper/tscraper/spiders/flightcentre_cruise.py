...
# Accept ANY /cruises/<slug> (slug can include digits and hyphens). We match only the PATH (no query).
ALLOW_PATH = re.compile(r"^/cruises/[A-Za-z0-9\-]+/?$")


class FlightcentreCruiseSpider(scrapy.Spider):
    name = "flightcentre_cruise"
    allowed_domains = ["cruises.flightcentre.co.nz"]
    start_urls = [f"{BASE}/", f"{BASE}/cruises"]
    custom_settings = {
        "DOWNLOAD_DELAY": 0.8,
        # Keep robots obeying; if the domain disallows, you’ll see zero results.
        # "ROBOTSTXT_OBEY": False,  # uncomment ONLY if you have permission to ignore robots.txt
    }

    def __init__(self, seeds: str = "", *args, **kwargs):
        """
        Optional: pass a space-separated list of URLs to force-visit details while testing.
        Example:
          scrapy crawl flightcentre_cruise -a seeds="https://cruises.flightcentre.co.nz/cruises/portugals-river-gold-lisbon-to-porto-viking-hemming-2025-11-08-2/ https://cruises.flightcentre.co.nz/cruises/tahiti-the-society-islands-le-paul-gauguin-2025-11-08-2/"
        """
        super().__init__(*args, **kwargs)
        self.seed_urls: List[str] = [u for u in seeds.split() if u.strip()]
    ...
    def _allowed_detail(self, href: str, base: str) -> Optional[str]:
        if not href:
            return None
        ...
        try:
            ...
            return None
        # Only same host, and path must look like /cruises/<slug>
        if p.netloc != urlparse(BASE).netloc:
            return None
        if ALLOW_PATH.match(p.path):
            return abs_url
        return None

    # ---------------- crawl ----------------
    def start_requests(self):
        # Seeds (explicit detail pages for testing) – keep query strings
            ...
            yield scrapy.Request(
                ...
                meta={
                    ...
                    "playwright_page_methods": [
                        ("wait_for_load_state", "networkidle"),
                        ("evaluate", "document.querySelector('button:has-text(\\'Accept\\'), #onetrust-accept-btn-handler')?.click?.()"),
                        # Some lists lazy-render; give it a moment
                        ("wait_for_timeout", 1200),
                    ],
                },
                dont_filter=True,
    ...
    def parse(self, response):
        # 1) Grab canonical detail links already on the page
        for href in response.xpath("//a/@href").getall():
            allow = self._allowed_detail(href, response.url)
            if allow:
                yield scrapy.Request(
                    allow,
                    callback=self.parse_detail,
    ...
    def parse_detail(self, response):
        ...
        item = PackageItem(
            ...
            url=response.url,
            title=title,
            destinations=[],  # add mapping if you find a consistent field on these pages
