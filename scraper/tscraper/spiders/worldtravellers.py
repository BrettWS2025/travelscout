...
ALLOW = ['^/deals?/[a-z0-9-]+/?$']
DENY  = ['/deals/?$', '/category/', '/blog/']

class WorldtravellersSpider(scrapy.Spider):
    name = "worldtravellers"
    allowed_domains = ["worldtravellers.co.nz"]
    start_urls = [f"{BASE}/holidays"] if "worldtravellers"!="worldtravellers" else [f"{BASE}/deals"]
    ...
    def parse_detail(self, response):
        ...
        yield build_item("worldtravellers", url, title, price, currency, nights, sale_end, text)
