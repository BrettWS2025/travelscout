...
ALLOW = ['^/deal/[a-z0-9-]+/?$', '^/holidays/(?!destinations/)[a-z0-9-]+/[a-z0-9-]+/?$']
DENY  = ['/holidays/?$', '/destinations/', '/search']

class HelloworldSpider(scrapy.Spider):
    name = "helloworld"
    allowed_domains = ["helloworld.co.nz"]
    start_urls = [f"{BASE}/holidays"] if "helloworld"!="worldtravellers" else [f"{BASE}/deals"]
