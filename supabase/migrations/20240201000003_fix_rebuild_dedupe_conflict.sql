-- Fix rebuild_nz_places_final to handle dedupe_key conflicts
-- When multiple places have the same dedupe_key, keep only one (prefer the one with admin area info)
