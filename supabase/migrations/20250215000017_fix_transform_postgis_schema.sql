-- ============================================================================
-- FIX: Update transform_walking_experiences to use schema-qualified PostGIS functions
-- ============================================================================
-- Purpose: In Supabase production, PostGIS functions are in the 'extensions' schema.
-- This migration fixes the function to explicitly qualify all PostGIS function calls
-- with the 'extensions.' schema prefix to ensure they work in both dev and prod.
-- ============================================================================
...
BEGIN
    ...
    SELECT
        ...
        -- Use schema-qualified PostGIS functions
        CASE 
            WHEN loc.x IS NOT NULL AND loc.y IS NOT NULL THEN
                extensions.ST_Y(extensions.ST_Transform(extensions.ST_SetSRID(extensions.ST_MakePoint(loc.x, loc.y), 2193), 4326))
            ELSE NULL
        END AS latitude,
        CASE 
            WHEN loc.x IS NOT NULL AND loc.y IS NOT NULL THEN
                extensions.ST_X(extensions.ST_Transform(extensions.ST_SetSRID(extensions.ST_MakePoint(loc.x, loc.y), 2193), 4326))
            ELSE NULL
        END AS longitude,
    ...
    LEFT JOIN LATERAL (
        ...
        WHERE admin_level = '6'
            ...
            AND extensions.ST_Within(
                extensions.ST_Transform(extensions.ST_SetSRID(extensions.ST_MakePoint(loc.x, loc.y), 2193), 4326),
                geometry
            )
        ORDER BY extensions.ST_Area(geometry) ASC -- Prefer smaller (more specific) districts
        LIMIT 1
    ) district ON true
